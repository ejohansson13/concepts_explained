...
Immediately after the skip connection has concatenated our images atop one another, doubling our number of channels, we repeat the same process as in our contracting path. The first convolution stage receives as input our 104x104x512 concatenation of decoder stage and encoder stage images. It outputs a 102x102x256 image stack, assimilating the information gained from the concatenating stage of the skip connection. We then run our output matrix through an elementwise ReLU, and repeat with 
