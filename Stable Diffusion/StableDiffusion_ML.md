...
The conditioning plays a critical role in guiding the U-Net's denoising of the latent towards a definite destination at inference time. During training, we are afforded the luxury of a pre-determined destination with our provided clean latent. While provided this luxury, it's important for the network to correctly train the weight matrices for its cross-attention mechanisms. 
