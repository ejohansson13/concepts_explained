...
Prompting our diffusion model requires the network's understanding of the prompt. We need an encoder to convert our prompt into embeddings which can interact with our latent embeddings in the U-Net. For the text-to-image case, this would be a text encoder but, alternative encoders can deliver prompt in different media while remaning compatible with the LDM architecture. The original LDM literature utilized BERT, but more recent literature has demonstrated that the performance of an image synthesis model [is strongly tied to the performance of its text encoder](https://arxiv.org/pdf/2205.11487.pdf). For this reason, newer diffusion models [have opted for more powerful pre-trained text encoders](https://arxiv.org/pdf/2307.01952.pdf).

The conditioning plays a critical role in guiding the U-Net's denoising of the latent towards a definite destination at inference time. During training, we are afforded the luxury of a pre-determined destination with our provided clean latent. While provided this luxury, it's important for the network to correctly train its query, key, and value weight matrices for cross-attention. Cross-attention occurs at every layer of the U-Net, with differing tokens in the textual prompt impacting the feature embeddings that constitute our latent. Conditioning via cross-attention 
